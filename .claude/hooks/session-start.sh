...
if [ -n "$built" ]; then
  ...
  echo "graphify graph: built from $built, $behind commit(s) behind HEAD (git hooks rebuild code per commit and branch switch, not per pull; non-zero right after a pull is normal until the next commit or branch switch; otherwise see ~/.cache/graphify-rebuild.log)"
else
