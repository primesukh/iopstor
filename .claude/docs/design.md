...
iopstor/templates/blocks/     one <section> template per block type (18)
...
- **A spacer and a divider zero `.section`'s padding, and tie `.column>.section+.section` to beat it** (2026-09-10). An empty section is 160px tall before anyone picks a height, and inside a column the stacking gap is `padding-top` on the section that *follows* — a (0,3,0) rule a bare `.spacer` (0,1,0) loses to, which made a Small spacer 40px and doubled the gap under it. Tidying `.column>.section.spacer` back to `.spacer` silently breaks it. The four heights are `.spacer.sp-*`, not bare `.sp-*` like `.fx-*`, because `height` is a plausible future field on another block.
