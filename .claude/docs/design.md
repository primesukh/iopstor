...
- **One padding for both hero shapes**: `.hero-dark` always carries `.hero`, so `.hero{padding:96px 0 80px}` (64/56 on a phone) is the only place it is set — do not reintroduce a `.hero-dark` override (client, 2026-09-10).
- **Hero rotator is CSS** (`--n`/`--i`, one keyframe set per picture count: 2 and 3 exist). The active dot is a **solid** blue bar for its whole turn — both keyframe stops in the turn hold identical values so nothing interpolates; it used to fill progressively and the client read that as a countdown timer (2026-09-10).
