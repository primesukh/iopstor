...
- **Logo keeps the artwork's colours in both bands**: header white (client, 2026-09-08), footer dark but no longer knocked out (client, 2026-09-10). `.site-footer .brand img` no longer exists — nothing recolours the logo. A near-black half of a wordmark reads faintly on `--black`; the client chose that over an inverted logo.
- **Logo is a fixed box, not a fixed height**: `.brand img{width:150px;height:22px;object-fit:cover}`, the logo's only rule (2026-09-10). The mock's `height:22px;width:auto` counted an uploaded file's own padding as logo — the live 2172x724 PNG has its wordmark in the middle 271 rows and rendered at 8px. `cover` crops the box full so the file's margins stop mattering; the 6.8:1 box shaves ~1px a side off the original 180x26 artwork, and a square logo drawn edge to edge loses its top and bottom.
