...
| Deploy | Dokploy → `Dockerfile` (python:3.13-slim, `flask migrate && gunicorn`; workers from `GUNICORN_CMD_ARGS` — `-w 2 --threads 8 --preload` by default, `-w 30` in Dokploy; the app is stateless across workers, TECHNICAL.md §15) |
