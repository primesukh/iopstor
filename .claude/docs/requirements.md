...
| 2026-09-21 | **The logo is shown whole, not cropped to fit** — the client uploaded a new logo in production and the header read "OPSTO", the I and R cut off. The header had been sizing every logo into one fixed 150x26 box, which fits only that shape. Offered three ways (show the whole logo / a Settings choice / re-export the logo to the box's ratio) and they chose showing the whole logo. **One thing to know when uploading:** the logo is now measured as supplied, so a file exported with wide empty margins around the wordmark will look smaller than it should — trim it before uploading. |
