...
| 2026-09-16 | **The technology-partner logos show in full colour, with a hover lift** — not the mock's greyed, faded strip. The mock (and the theme after it) ran the row at `grayscale(1)` and `opacity:.7`, with colour returning only under the pointer; the client asked for the marks as their own files carry them. Pointing at one now lifts it 2px, the same movement every other card on the site makes |
