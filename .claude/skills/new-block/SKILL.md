...
2. **Field names first** — five dicts key off them. Reuse an existing key (`text`, `heading`, `cta_label`/`cta_url`, `link_label`/`link_url`, `media_id`) and its widget, label and text-order come free. `EDITOR["labels"]` takes a `"<type>.<field>"` override the way `widgets` always has (2026-09-22, for `hero.dark`) — `labelFor(type, field)` tries it before the bare key. **Reach for it only when one key genuinely means two things** (`dark` is the hero's *layout* and the testimonial's *colour*): the bare label is what keeps a reused key consistent, `link_label` reading "Header link text" everywhere is the feature, and NON-TECHNICAL.md quotes these strings, so either way update it. Widening the shared label, or picking a key whose label already fits, still beats a second override. Only the canvas placeholder can differ per block without any of this, via `fe('key', ph='…')`.
3. **`EDITOR`** in the same file:
   ...
   - `labels` for any new key that is not readable as-is (bare key; `"<type>.<field>"` only when one key means two different things)
