...
3. **One schema edit per file.** Idempotent where Postgres allows it (`ADD COLUMN IF NOT EXISTS`, `CREATE INDEX IF NOT EXISTS`). NOT NULL columns carry a `DEFAULT`. `updated_at` uses the `moddatetime` trigger the way `0001` does.
   - **A content migration** — rows rewritten, no column changed (`0011`) — is still a numbered step, with rules of its own. Match by **content, never by id**: production has its own ids, and a page somebody already rewrote by hand matches nothing and is left alone. When it changes `posts.blocks`, it rewrites `post_drafts.blocks` **and clears `post_drafts.state`** in the same statement, or the stored shared document puts the old shape straight back on the next open. When what `quillKeeps()` accepts has changed, it strips `data._rich` from every `rich_text` block, because the verdict is stored and never recomputed. Say "safe to run twice" only when the second run really matches nothing. You cannot execute it here: run the same transformation in Python against the real rows, read-only, and name the matches in the hand-over (PR #70 found exactly one target per page).
