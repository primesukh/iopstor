...
| **Related services** | Services only. The row of buttons at the bottom of a service page — *Storage solutions* on a group's page, *Other Storage services* on one of its services. Untick **Show the related services at the bottom of this page** to remove it from this page only. **Heading** replaces the words above the buttons (leave it empty and they are written for you — the box shows what they would be); **Text under the heading** adds one short line; **Background** is *Light grey* as it has always been, or *Page background*, *Dark* or *Blue*; and under **Services it lists** untick any service you do not want offered here. A service published later is added on its own, so you do not have to come back to every page. Preview shows all of it as you change it. |
...
you swipe. Clicking an appliance still opens its page. How many are in the line is the section's **Limit** —
open its **⚙** to change it. To go back to rows of cards, set **List style** to *Cards* there.
...
same Services pages, and the *All services* link is still on the right of the heading. The small arrow beside
the count points down when a line is shut and up when it is open. If you prefer
the cards back, open the section's **⚙** and set **List style** to *Cards*.

**Clicking a service line on the home page opens its page.** Yes — since 24 September an open line is one big
link to that group's page, so there is no need to aim for the small *All Hyper Converged Media* words any more
(they are still there). The sub-service buttons, such as *Proxmox* and *VMware*, still open their own pages. On a
...
**How do I hide or change the "Other Storage services" buttons at the bottom of a service page?** Open that service and look under **Related services** in the panel on the right. Untick **Show…** to remove the row from that page, or change its heading, add a line of text under it, give it another background, or untick the services you do not want listed there. Each page has its own settings, so changing NAS leaves DAS and SAS as they were.

**I gave a service a Featured image and the Services page did not show it.** It does now: the picture sits at the top of that service's card, above its number and name, with its sub-services still beside it. A service with no picture keeps the card it always had, so you can add pictures one service at a time. The picture is trimmed to a wide strip, so choose one whose subject is in the middle.
