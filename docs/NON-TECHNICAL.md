...
**On my phone the menu was one huge list, and the partner logos were enormous.** Both fixed, and neither needs anything from you. Tapping the menu button now shows a short list of headings — *Storage*, *Cloud*, *AI* and so on — each with a small arrow; tap one and just that group's pages open underneath it. Everything else in the menu is now reachable without scrolling past twenty links. And the technology partner logos are three to a row instead of two, so they read as a row of partners rather than a wall. On a computer both look exactly as they did before.
