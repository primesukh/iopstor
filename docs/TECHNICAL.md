...
- **Everything scroll-driven is behind one `@supports (animation-timeline: view())`, and that gate is load-bearing.** A browser that parses `animation-timeline` but cannot resolve `view()` holds a `both`-filled animation on its opening frame for ever: `fx-rise` left the whole section at `opacity:0` and `fx-count` left an empty box where a number should be. Firefox 140 ESR does exactly this. Off the gate all three are plain, static content, which is the honest fallback — and it is why the digits live in the HTML rather than only in `content: counter(...)`.
...
- Scroll-driven effects need `animation-timeline: view()` (Chrome 115+, Safari 26+, Firefox 144+). Older browsers get the static content, on purpose — see §12.
