...
- **Ceilings** (`# ponytail:` in `stress.py`): the controller thread lives in whichever worker took the request, so a recycled worker freezes the run at its last flush; the only guard on the target URL is the scheme (no SSRF allow-list — owner-only behind the office-only admin, and dev→prod on the LAN is the point); inputs are clamped to `MAX_WORKERS` (200) of each and `MAX_SECONDS` (300). When the target is this same instance the load competes with the worker serving it, so the figures read heavier than real traffic — that is a rough gauge, not a benchmark; and the real capacity ceiling it will surface is PostgREST's pool (§15 step 4: 240 request slots in front of `PGRST_DB_POOL=10` 504s under a burst), not gunicorn.
