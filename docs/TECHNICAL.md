...
**The footer's user block truncates, it does not wrap.** `.adm-me-t b` (the name) and its `span` (the email) are `white-space:nowrap` + `text-overflow:ellipsis`, with the full address in a `title`. That needs `min-width:0` on **both** `.adm-me` and `.adm-me-t`: a flex item defaults to `min-width:auto`, refuses to shrink below its content, and the ellipsis silently never appears. The name itself is the `display_name()` Jinja global (`__init__.py`) — `users.name` when set, otherwise the email's local part with `. _ -` turned to spaces and title-cased, because `users.name` defaults to `''` and `flask create-admin` never fills it. `admin/users.html` calls the same global, so the table and the sidebar cannot disagree.

**`POST /admin/users/name`** (`admin_ui.user_name`) writes that name. It updates **`g.user["id"]` only** — it never reads an id from the form, so it cannot rename anyone else, and `ui_required` already checks the CSRF field. It is gated `admin` because the form lives on the admin-only Users screen; an editor keeps the derived name until it gets its own `/admin/account` page (marked `# ponytail:` in `admin_ui.py`). No migration: `users.name` has existed since `0001_initial.sql`. The field carries no placeholder — the derived name in a greyed-out box reads like a value already saved — so the hint lives in the label instead. Both forms on that screen are `autocomplete="off"`, and *Temporary password* is `autocomplete="new-password"`: an email field beside a password field is a login form to a password manager, which was filling the signed-in admin's own saved credentials into the invite.
