...
The selection is a `<form>` wrapped round `.tiles-m` with a checkbox per tile and **no script at all**. The tile had to stop being an `<a>` (a checkbox inside an anchor is invalid and the anchor eats the click): it is now a `div.m-tile` holding `label.m-pick` and `a.m-open`. Both states are CSS — `.m-tile:has(.m-pick input:checked)` turns a ticked tile red, and `form:has(.m-pick input:checked) .m-bulk button` is what arms the Delete button, which is grey until something is ticked. `:has()` was already load-bearing in `admin.css` (`.role-card`, `.admin-main:has(#post-form)`), so this adds no new assumption. A "select all" box and a live count would need `admin.js`, which does nothing on this page today; they are not there.
