...
- **Why the load runs from inside the app, in a background thread.** `start()` validates and clamps the inputs, registers the run and spawns one daemon controller thread, then returns at once — the POST does not block a worker for the run's length. The controller spawns `visitors + attackers` worker threads (each loops back-to-back until the deadline or a stop flag), aggregates counters under a lock, and flushes a snapshot every 0.5s. Stdlib only — `urllib.request` for the requests, `concurrent`-free plain `threading`, `sqlite3` for the store — because `pip install` is denied and no load tool (`ab`, `wrk`, `hey`) is in the image; `urllib` is the same client the realtime proxy already uses.
- **Why progress lives in a sqlite file on `/dev/shm`.** The ~30 gunicorn workers are separate processes, so a run started in one and polled from another (`GET /admin/stress/progress/<id>`, a GET, so it adds nothing to the audit accounting) need shared memory. This is throttle.py's trick exactly: tmpfs, wiped on redeploy, which is a load run's whole lifetime. Nothing here is application data. `STRESS_DB` is a module constant, **not** an env key, specifically so it stays out of `docker-compose.yml`'s `x-app-env` and the container-env test.
...
- **Ceilings** (`# ponytail:` in `stress.py`): the controller thread lives in whichever worker took the request, so a recycled worker freezes the run at its last flush; the only guard on the target URL is the scheme (no SSRF allow-list — owner-only behind the office-only admin, and dev→prod on the LAN is the point); inputs are clamped to `MAX_VISITORS` (10000), `MAX_ATTACKERS` (1000) and `MAX_SECONDS` (300) — but those are the "push until it breaks" bound, not a clean-concurrency promise: it is one OS thread and one socket per worker, so the real ceiling is the open-file-descriptor limit (often 1024) and thread/memory overhead, and past a few hundred to ~1000 the surplus turns into no-answer errors rather than more concurrency. True concurrency in the thousands would need an async client, a barred dependency. When the target is this same instance the load competes with the worker serving it, so the figures read heavier than real traffic — that is a rough gauge, not a benchmark; and the real capacity ceiling it will surface is PostgREST's pool (§15 step 4: 240 request slots in front of `PGRST_DB_POOL=10` 504s under a burst), not gunicorn.

Guarded by `test_run_load_hits_a_real_server_and_the_honeypot_leaves_no_row` (§13), which runs the engine against a throwaway stdlib server and proves requests are sent, statuses tallied, real pages answer 200 and every lead post arrives with the honeypot filled so nothing is stored.
...
**The stress engine (`iopstor/stress.py`) is covered offline**: `test_validate_target_*` (scheme-only URL guard), `test_clamp_*`, `test_percentile_is_nearest_rank_in_ms`, `test_progress_store_roundtrips_and_stops` (the `/dev/shm` store on a `tmp_path` file), and `test_run_load_hits_a_real_server_and_the_honeypot_leaves_no_row` — the measured one, which stands up a throwaway `http.server`, runs `stress.start()` at it for ~2s and asserts requests went, real pages answered 200 and every honeypot lead post was dropped without a row. No Supabase, so it lives in `test_offline.py`.
...
- The stress-test run (§12) is bounded but coarse: its controller thread lives in whichever worker took the request, so a recycled worker freezes the run at its last `/dev/shm` flush; the only guard on the target URL is the scheme, no SSRF allow-list (owner-only behind the office-only admin, and dev→prod on the LAN is the point); inputs clamp to 10000 visitors / 1000 attackers / 300s, but that is a thread-per-worker design so the true ceiling is file descriptors (often 1024) and memory, not the clamp — past a few hundred the surplus is no-answer errors; and a self-test's load competes with the worker serving it, so those figures read heavier than real traffic — a gauge, not a benchmark. What it will actually surface first is PostgREST's pool, not gunicorn (§15 step 4).
