...
**The design has four animations, all on menus.** `drop` (the mega panel at .22s and the Company drop-down at .2s, `cubic-bezier(.2,.7,.2,1)`), `fade` (the mega's right pane, .2s ease-out, as the pointer moves down the group list) and `slide` (the mobile sheet, .25s ease-out). Every one is `both`-filled so it holds its end state. **Nothing else moves** — the hero, the bands and the cards are static in the mock, and adding motion there would be invention, not implementation. Hover work is `transition` at .15s.
