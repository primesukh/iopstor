...
- **Slug** — `#post-slug` is `readonly`; typing in `#post-title` live-fills it with a JS mirror of `db.slugify()` while the post has no saved slug. An *Edit* button unlocks the field after a confirm, for the deliberate URL change. The field also carries `data-taken` — every slug of this post type except this post's own, built by `_form_context()` from the one `siblings` select that also feeds the *Parent* dropdown (space-separated: slugs are `[a-z0-9-]`, so no JSON is needed, and it is an attribute rather than `#editor-data` because `initSlug()` runs before `initBlocks()` parses that blob). On every keystroke in either field, `check()` compares the slugified value against that list: a clash turns `#slug-hint` red and names the next free address via `freeSlug()`, a mirror of `db.unique_slug()`, and calls `setCustomValidity()` so **Save** is blocked once *Edit* has unlocked the field — a `readonly` input is barred from constraint validation, so before that the message alone does the work. The list is only a page-load snapshot, so `apply_post()`'s `409 slug already in use` remains the authority — it still generates the slug from the title when the submitted one is empty, and leaves an existing slug alone. Its `fields.slug` is now a sentence (*“about-us” is already in use — try “about-us-2”*) rather than the bare slug, because `_save()` prints `fields[k]` straight into the form's error box the way it does for every other field.
