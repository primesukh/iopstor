...
**Every archive head is the light one, and the row of figures sits on the page** (client, 2026-09-21). `archive.html` alternated: Services, Case Studies, Partners and Products on `band-dark`, Blog and the reading types on white. That was a faithful transcription of the mock, which draws exactly those four screens on `#0a0d12` (`website_assets/mock-website.html:230,299,364,485`) and Blog on `#fff`. The client overrode it — on a site that is otherwise light those were the largest dark areas, and the alternation read as an inconsistency rather than as the intended rhythm. `requirements.md` carries the decision; **the mock is no longer the reference for this and putting `band-dark` back on `.arch-head` would be undoing a client call, not a regression.**

`band-dark` itself is untouched and still earns its keep — `post.html`'s title banner writes it, and the Dark **tone** an editor can set on any section is a separate class (`t-dark`). What changed is that `archive.html` and `blocks/stats.html` stopped writing it.

**Two things the stats block lost with that class, neither of them the background.** Its figures were `var(--white)` and its labels `var(--muted-dark)`, both invisible on the new light band, so they are now `--ink` and `--muted`. And `.band-dark.section{padding:40px 0}` was what made the strip slim — without a rule of its own it would have inherited `.section`'s 80px and doubled in height, so `stats.html` writes `stats-band` and `.stats-band.section` carries the same 40px at the same specificity.

**And the override that a screenshot would not have caught:** an editor can still set **Tone: Dark** on a stats block, and with the colours moved to light that control would have silently made the numbers unreadable. `.t-dark .stats strong,.band-dark .stats strong` and the matching label rule keep it legible. Measured in one render, both tones at once: light → figure `rgb(46,49,51)`, label `rgb(91,102,117)`; dark → figure `rgb(255,255,255)`, label `rgb(174,182,194)`; padding 40px in both.
...
- **`columns` is the only block that holds other blocks.** `data.cols` is a list of columns, each an ordered `[{type, data}]` of its own; `data.widths` is `"50/25/25"` or blank. It uses the same `extra` hatch as `post_list`, so `columns.html` is handed a `col(n)` callable that renders column `n` and a `widths` string — no recursion is wired into Jinja, and `__init__.py` / `public.py` are untouched.
  - **One level only.** `validate_blocks(..., nested=True)` rejects `NEVER_NESTED = ("columns", "hero")` inside a column: a grid inside a grid is how an Elementor page becomes unmaintainable, and a hero is a full-bleed band, and when it opens the page it owns the page's only `<h1>`. `admin.js` mirrors the tuple so the inserter never offers them and a drag into a column is refused.
...
`POST /leads` is the one write anybody on the internet can reach, so **every field it stores is capped** — `name` 200, `email` 300, `phone` 50, `company` 200, `message` 5000, and the keys it does not know by name (`interest`, `users`, `product`, … → `leads.data`) at 20 keys of 200 characters each, stringified. Without the last two the only bound is `MAX_CONTENT_LENGTH`, which is 20 MB because an *upload* needs it: a single POST could store 20 MB in `leads` and, because `db.insert()` puts the whole created row through `_diff({}, created)`, another copy in `audit_log` — which is append-only by trigger and has no retention job, so neither copy can be deleted afterwards. `known` is filtered out *before* the 20 are counted, so the fields read by name never spend that budget; the site's own forms put two keys in `data` (`interest`, `users`), well inside it. Guarded by `test_a_public_lead_cannot_store_more_than_it_is_allowed_to` (offline), which also holds the honeypot's accept-and-drop.
...
- **A post** (`_md_post()`) → YAML front matter (`title`, `url`, `type`, `published`, `updated`,
  `description`), then the body. Exactly one `#`, however the page is built — two questions, and
  conflating them is the trap. `owns_head()` decides whether the page draws its own title, and rides
  on to `blocks_md(h1=)` so a hero that is not the page's heading writes `###`. Whether `_md_post()`
  writes the `#` itself is the narrower question of whether a hero is about to, and **only a `hero`
  ever does** — `blocks_md()` gives a `columns` block `##`. So a columns-led page keeps its `#` here
  even though its HTML has no `<h1>` at all; that gap is a defect in the page, and mirroring it would
  only lose the heading twice. `test_the_markdown_twin_opens_exactly_one_heading` pins all four shapes.
...
`0015_seed_hero_off_case_studies_and_events.sql` is the shape to copy for **undoing a row the seed got wrong**,
and its two rules are worth keeping. (1) **Match the shape, not the id or the slug.** It clears only a lone `hero`
block whose data has no key but `heading` — the seed's exact output — so a hero anyone has since filled in survives,
and production, which nobody here can see, decides for itself which rows qualify. Every test is also written so no
value can make it *raise* rather than simply not match: `jsonb_array_length()` throws on a `blocks` that is not an
array and a `WHERE` is evaluated in any order, so a guard in front of it saves nothing — `blocks->1 IS NULL` says
the same thing and returns NULL on anything else. (2) **The draft goes with the blocks, in one statement.** It is a
`WITH cleared AS (UPDATE … RETURNING p.id) DELETE FROM post_drafts WHERE post_id IN (SELECT id FROM cleared)`, so
the delete can only touch rows the update actually changed; re-matching on `blocks = '[]'` afterwards would also
...
**The logo is shown whole: a fixed height, and the width follows the file** (client, 2026-09-21). `.brand img` is `height:26px;width:auto;max-width:200px;object-fit:contain`, shared by the header and the footer — the footer rule carries nothing but the knock-out filter. **This reverses the 2026-09-10 decision and the reason that decision existed is still true**, so read both. It was a fixed `150×26` box cropped with `object-fit:cover`, because a fixed height alone lets whitespace baked into a PNG count as logo: a 2172×724 file an editor had uploaded rendered its wordmark at 8px instead of 19px. `cover` made the file's own margins stop mattering. But a fixed box only ever fits artwork at its own ratio — 150×26 is **5.77:1** — and it crops everything else *silently*. An editor uploaded a wider logo in production and the header read **"OPSTO"**, the I and R gone. The previous version of this paragraph said the 180×26 artwork "loses about a pixel off each side"; it loses **30px of 180**, about 15 each side, and that understatement is a large part of why the risk read as acceptable. (It also said `150×22` and `6.8:1` while the rule had been `150×26`/5.77:1 for some time — stale on both numbers.) Measured in a browser across three files in `website_assets/`: at 6.92:1 the old rule rendered 5.77:1 (the reported crop), at 5.71:1 it was imperceptible, and a square logo showed a 150×26 slice of its middle. The new rule renders all three at their own ratio. **The cost is the reverted one and it now falls on the editor:** a logo exported with wide empty margins renders small, because those margins are counted again. `NON-TECHNICAL.md` says so beside the upload box, and the case is live rather than theoretical: the file still configured on the practice site is that same 2172×724 upload, whose wordmark is 271 of its 724 rows, and it now renders 78×10. Trimmed artwork is the answer; `cover` is not. **The box stays the design's own 150×26 and only the fit changed** (`cover` → `contain`). The first version of this fix used `max-width:200px`, which was an unnecessary widening — a wide logo then rendered about a third larger than the design's footprint, and the client saw it immediately. Measured from their two screenshots at the same scale: 218×29 against 164×21, i.e. 200×26.6 against 150×19.3 in CSS pixels, the new one pinned exactly to that 200 cap. At 150 a 7.5:1 logo renders 150×20, which is the size the header always read at. Whichever edge runs out first decides: a wordmark wider than 5.77:1 is capped by the width and comes out under 26px tall; a squarer one is capped by the height. So the footprint can never exceed 150×26 and the worst outcome is a smaller logo, never a cut one. `flex:none` stays because `.brand` is a flex container and the item would otherwise shrink at 390px. **`ponytail:` no server-side trim** — Pillow is not in the venv, and one CSS line is not worth a dependency plus an upload hook; revisit if editors keep uploading padded artwork. `test_an_uploaded_logo_is_shown_whole_and_never_cropped` pins it.
...
**The services mega panel** is CSS only, like everything else on the public site. Its markup is a child of the Services `<li>`, absolutely positioned against `.site-header` — sticky is a positioned element, so it is the containing block, which is how the panel spans the viewport instead of the 1200px column. That is also why the Services `<li>` is `position:static` while every other one is `relative`: the Company drop-down has to anchor to its own item. **Opening is plain `:hover` / `:focus-within`**, so it works everywhere.

**It is one column per service group, and nothing switches** (client's design, 2026-09-21). `.mega-cats` is `grid-template-columns:repeat(auto-fit,minmax(180px,1fr))` over one `.mega-col` per group; the column head is the group's own link (`a.mega-g`, uppercase and small, still pointing at `/services/<group>` even though the design draws it as a plain label), and under it every child is a row with a 32px tinted glyph tile. Everything is on screen at once, so there is no visible-group state at all — the `:hover`/`:focus`/`:has()` switch this panel used to carry, and the `.mega-cats:not(:has(:hover,:focus))` "nothing chosen yet, show group one" default, are **gone with the two-column shape they served** rather than disabled. What remains inside `@media(min-width:961px)` is one rule, `.mega-pane{display:block}`.

**`auto-fit` over a wrapper, not `nth-of-type` over flat siblings**, and that is the part to keep. The groups are content, so counting them in CSS puts a ceiling on how many services the menu can hold; the old shape needed eight `nth-child` rules and broke on a ninth service before index pairing replaced them. `.mega-col` exists so the grid can place a group as one item — it is the only structural addition, and it goes **around** the four parts, never between them (see the phone chain below).

**The glyphs are a `<symbol>` sprite in `base.html`, not a script.** The design ships them as `assets/icons.js`, which writes an inline `<svg>` per item; the public site carries one first-party script and this is not it, so the seventeen service icons, the five group icons and a fallback `dot` are markup, drawn once per page and pointed at with `<use href="#svc-…">`. Two things that bite: the sprite is hidden with `.svg-sprite{display:none}` and **not** `hidden`, because the UA rule for `hidden` is HTML-namespaced and an inline `<svg hidden>` still reserves 300×150 (the footer's social sprite hit this in 2026-09-11); and the stroke properties live on the *using* `<svg>` in `site.css`, the way `admin/base.html`'s sprite does it.

**Which glyph a service gets is `public.service_icon()`**, and the value is whitelisted because it lands in a `use href` attribute — `section_class()`'s rule. An editor's **Icon** box wins when it names one we ship, otherwise `SERVICE_ICONS` (a slug → symbol map, the design's own choices) answers, otherwise `dot`. That is a `# ponytail:` ceiling: it is presentation held in code rather than 22 rows of content, and the upgrade is to set `meta.icon` on every service and delete the map. Nothing breaks without it — a new or renamed service simply gets the dot. `_service_nav()` attaches the answer to each group and child so the whitelist is one function rather than a Jinja expression; it writes a key `db.tree()`'s other two callers do not read, which is safe and idempotent.
...
The checkbox and its label go **before** the group link, and the four stay adjacent. The phone's chain is `.mg-toggle:checked + .mg-row + .mega-g + .mega-pane`, **adjacent the whole way**, and a `~` would open every group below the one that was tapped as well. Since 2026-09-21 the four sit inside a `.mega-col` rather than flat in `.mega-cats`: that is what the desktop grid places, and it is deliberately a wrapper **around** the group and not an element between any two of the four — putting it in the middle breaks the phone and nothing else, which is the kind of change nothing complains about. Two tests pin it: `test_the_mega_menu_keeps_the_order_both_layouts_depend_on` on the markup and `test_the_phone_disclosure_chain_survives_the_column_wrapper` on the rendered header.
...
Everything that opens a pane **by pointer** lives in `@media(min-width:961px)`, and `.mega-pane{display:none}` stays outside it as the shared default — closed is what both layouts start from. The rule that made this urgent is gone with the group switch (`.mega-cats:not(:has(:hover,:focus)) .mega-g:nth-of-type(1)+.mega-pane` was (0,4,0) and out-specified the phone's plain `display:none`, jamming group one open on a touch screen), but the reason still holds for anything added later, and `test_a_pointer_only_menu_rule_never_escapes_the_desktop_query` now scans the whole file for a `:hover`/`:focus-within` rule that sets `display:block` outside a query rather than naming the two it knows about. **834 is already the phone sheet** — the breakpoint is 961, so `/theme-check`'s middle width never shows the desktop panel at all.
...
**A case study is an article too** (client, 2026-09-19). `post.html`'s `article` is `pt.slug in blocks.ARTICLE_TYPES` (`post`, `case_study`) — a case study is a customer's story, and it reads the way the blog does. **The template is only half of it:** the stacking lives in `site.css` keyed off the `pt-<slug>` class, so both rules list both types (`.pt-post .page-head,.pt-case_study .page-head` and `.pt-post .page-media img,.pt-case_study .page-media img`). A slug added to one and not the other renders stacked markup wearing the 440px banner crop meant for a card, which looks deliberate and is not — `test_a_case_study_stacks_like_an_article_rather_than_sitting_beside_its_picture` asserts both selectors for that reason. The date now shows on a case study, which is what "like the blog" means.
...
Nothing in that query applies at 760px and up, where the `pl-cols-*` count from `even_cols()` takes over — fourteen partners stay at 7×2 and 295px at 1440 and 834. A fifteenth makes it 8×2 (`pl-cols-8`), which is the same 140px of grid: the tracks narrow instead, measured 155px → 135px at 1440 and 103px → 89px at 834, so the marks go 123×34 → 103×28 and 71×20 → 57×16. The 834 number is the one to watch: 57px is legible in a screenshot but is the smallest mark the strip has ever drawn, and a sixteenth partner keeps it there.

**The strip sizes a mark by its height, so a logo's aspect ratio is the whole of how big it looks** (2026-09-21). `.card` is 64px tall with `10px 16px` of padding, so `max-height:100%` caps every mark at 44px and the width follows: at 2.9–3.6 : 1, which is what a horizontal lockup is, that is 128–159px wide, and it is the *width* the track then clips. A **stacked** lockup (mark above the word, ≈1.14 : 1) comes out ≈50px wide with a wordmark about 5px tall — which is what Seagate's `seagate_PMS_stacked_pos.png` did, at 3380×2967. The file's pixel count is irrelevant; nothing in CSS can make a square mark read as wide as a landscape one without changing the tile for every partner. **Ask for the horizontal lockup** — the library's own files are 590×202 to 2600×763 — and note there is no image resizing anywhere in the app (`db.get_media`), so an oversized brand-pack PNG ships whole to every visitor.
...
**A product's picture area is a fixed height, never a minimum** (2026-09-18). A product photo's aspect ratio is whatever the client sent — the tower is 600×375, the rack unit 518×135 — and `min-height` only grows the box for a picture tall enough to reach it. So the short one sat in a 176px box and the tall one in a 191px box, and because that box draws the `border-bottom`, **the divider and the title were 15px out of step between two cards sitting side by side**. The archive was worse: 217px against 180px, a 37px step.

Both are now `height` with the picture capped at `max-height:100%` and centred, so a card rules off at the same place whatever shape the next photo turns out to be: **190px** in the deck (150 for the picture + 20 above and below) and **216px** on the archive (160 + 28). Measured after: at 1440, 834 and 390 both deck cards report an img-box of 190, a divider at 191 and a title at 211; the archive's two boxes are 216 and 216.

`max-width:100%` went on with it. The deck was relying on flex-shrink to keep a 518px-wide picture inside a narrower card — true today, and silently false the moment that box stops being `display:flex`.

It is only `pl-product` that had this. The other picture types already pin a dimension: `pl-case_study` sets `height:150px` on the image, `pl-post` gives the box `aspect-ratio:16/9`, `pl-partner` a flat `height:64px`.
...
**That group must stay below `.pt-post .page-head,.pt-case_study .page-head`**, which draws a 1px `--line` hairline — a light rule across the bottom of a black band. Both selectors are (0,2,0), so it is a specificity tie and source order is the only thing deciding it, the same mechanism `.arch-head.band-dark` and the `pad-*` group already carry comments about. `test_the_banner_rules_sit_below_the_article_rule_they_have_to_beat` compares the two indexes rather than asserting the rule exists, because existence is not the property that matters.

**Both halves are required to draw anything** — `post.featured_media and meta_.get('_head_banner')`. A flag with no picture chosen would otherwise be a black box with a headline in it. And the picture appears **once**: `.page-media` gains `and not head_banner`, which is the two-pictures rule of design.md §9 seen from the other side. Measured in Firefox on the rendered band: background `rgb(10,13,18)`, `border-bottom:0px`, backdrop `opacity:0.18`, wrap `position:relative`, and every one of title / date / lead / breadcrumb / chip resolved light (`255,255,255` or `174,182,194`) — a screenshot of white-on-dark can look right while one of the five is still `--ink` and happens to sit over a dark part of the photograph.
...
**A type's short fields are page content, not a callout** (client, 2026-09-19). `.meta-strip` was a full-width `var(--grey)` band holding one white rounded card per field; the client asked for the value not to be highlighted, so both the band's background and `.details>div` are gone and the label/value pairing is all that is left. The band's `padding:32px 0` stays — it is the gap between the head and the writing, not decoration — and the grid stays too, so an event's three dates still sit in a row rather than stacking. This is shared: case studies, events, datasheets, services, products, partners and testimonials all draw their short fields through it. `test_a_types_short_fields_are_page_content_rather_than_a_callout` guards the three declarations.
...
**Text size is a real Quill format, and it applies to the selection** (client, 2026-09-19). Two faults, reported together, in two different paths.

**It spread to the whole section — on the *second* pick.** `setSize()` ran `execCommand("fontSize","7")` and rewrote each `font[size="7"]` into a `<span style="font-size:…">`. `savedRange` is a **live `Range`** whose boundaries sat inside that `<font>`, so moving the font's children out collapsed it — a boundary inside a removed node is reset to the parent — and unlike `applyLevel()` and `exec()`, `setSize` re-recorded nothing afterwards. The first pick was correct; the highlight then vanished; the second pick (*"Large was too much, try Larger"*) found a collapsed range, and `execLine()` widens a collapsed caret to `selectNodeContents(caretBlock())`. In a one-paragraph section that is the whole section. Worse in a block with no wrapper element, where `caretBlock()` returns **the field itself**. The fix is `exec` instead of `execLine` — `execCommand` with a collapsed caret already styles what gets typed next — plus **re-selecting the words across the surgery**, which is what makes a second adjustment work rather than merely not break.

**And it was switched off wherever Quill owned the field**, which is most blocks: `size.disabled = … || !!q || …`. Quill *does* ship a size format; it was unreachable. `attributors/style/size` is **import-only**, and the `formats/size` in the default registry is the **class** attributor (`ql-size`, whitelist `small|large|huge`). Both halves are therefore required — `Q.register(SizeStyle, true)` with `whitelist` widened to `TEXT_SIZES`, **and** `"size"` in `QUILL_FORMATS`, because `formats:` is a strip-everything-else list snapshotted into a private registry at construction. Either alone and a rem size fails `canAdd()` and vanishes silently.

**The style attributor, never the class one.** It emits `style="font-size:…"`, byte-identical to what the legacy path writes — so the two paths agree about the same paragraph, and `quillKeeps()`'s `marks()` (tags and classes) is unaffected. `.ql-size-*` exists only in `vendor/quill.core.css`, which the public page never loads; a class-based size would render in the editor and vanish on the site, the same trap `test_the_canvas_loads_quill_inside_the_iframe` guards for lists.

**`withSize()` runs for every Quill constructor, and the memo is on the constructor.** There are two copies — the canvas iframe's and the parent's offscreen `converter()` — and the iframe's is **a new object after every full repaint**, because `canvasFull()` replaces `srcdoc`. Register one and not the other and a peer's paragraph renders one way in the editor and another through the converter.

**Making a line a heading still clears its size, and on the Quill path that needed saying twice.** `applyLevel()` does it for legacy and explains why: the level *is* the size, and since Size is disabled on headings a leftover span is a dead end with no way back from the toolbar. But `qfmt("header", …)` returns true in a Quill field, so `applyLevel` never runs there — `clearSizeOnLine()` applies the same rule with `formatText(lineStart, line.length(), "size", false)`. The line, not the selection: otherwise only the part the caret covered recovers.

**One policy, two enabled-rules, and that is deliberate.** `caretBlock()` cannot see a heading inside a Quill field — Quill mounts `div.ql-editor` inside `[data-f]`, and `caretBlock()` walks up to the child of `savedField`, so it returns that `DIV` and the `/^H[1-6]$/` test is always false there. The Quill branch of `syncBar` applies the rule from `now.header` instead. Leave both.

**Undo belongs to the shared document, not to an editor** (client, 2026-09-19). `initShared()` builds a `Y.UndoManager` over `YB` with `trackedOrigins: new Set([YORIGIN])`, and that one object answers everything ↶ and ↷ do. Scoped to the document, so it survives the repaints that used to destroy a per-editor stack; scoped by origin, so **a peer's ops are not on it and undo cannot revert their work** — that is a property of the data structure, not a check somebody has to remember. It covers structure and prose alike because both live in this one `Y.Doc`.

**The repaint is free and that is why the origin matters.** An undo's transaction carries the UndoManager as its origin, which is not `YORIGIN`, so `reconcileIn()` does not skip it: it takes the same shape branch a peer's structural edit takes — `MODEL = fromY()` then `canvasFull()`. No new painting code exists for undo.

**Quill keeps no history of its own** (`HISTORY_OFF = {maxStack:0, userOnly:true}` on all three `new Quill()` calls). That is what y-quill expects when a `Y.Text` is the source of truth, and it removed two faults nobody had reported: Quill's default `userOnly:false` recorded a **peer's** delta on your stack, so ↶ reverted their sentence and the binding broadcast the revert to everyone; and `mountQuill`'s seeding `dangerouslyPasteHTML` was recorded too, so undoing far enough emptied the block. `maxStack:0` leaves Quill's own Ctrl+Z bindings registered but permanently inert rather than fighting ours.

**One action, one step — and the floor is the debounce.** `reconcileOut()` batches on 120 ms, so transactions are timer ticks and not actions. `markStep()` (`step()` + `markDirty()`) is called by the ten discrete structural mutators so two clicks never merge; typing deliberately calls plain `markDirty()` and coalesces on `captureTimeout:500`, which is the behaviour Quill's `delay:1000` always gave. The settings panel fires one listener for `input|change|click`, so it steps on everything **but** `input` — picking from a dropdown is an action, typing into its text box is not. Two actions inside one 120 ms window are still one step and nothing can split them; §17.

**Housekeeping must not reach the stack**, and this was measured rather than reasoned: `seedDoc()` writes the whole page under `YORIGIN`, so without `UNDO.clear()` the seed was the first undo step and ↶ on a freshly opened page emptied it. `dedupe()` clears for the same reason — undoing a repair would put the duplicate sections back. `tests/reconcile.mjs` asserts `canUndo()` is false on an untouched document, which is the assertion that caught it.

**Undoing a deleted rich-text block would otherwise return it as plain text.** `yData` refuses to write `html` because the `Y.Text` holds a Quill *delta* while `toJSON()` is bare words, so `fromY()` normally reads formatted HTML back out of `MODEL` by `_id` — and after undoing a delete the block is in the document and *not* in `MODEL`. `fromY()` therefore falls through to `ytextAt(_id)` and renders the delta through `converter()`, the same offscreen Quill `mirrorProse()` already uses for a peer's words, so the output is byte-identical to a mounted editor's.

**Shortcuts are bound in both documents.** `undoKey()` handles Ctrl/⌘+Z, Ctrl/⌘+Shift+Z and Ctrl+Y; `wireDoc()` attaches it to the canvas document (once per document, and `srcdoc` hands back a new one each repaint, so it never stacks up) and `initBlocks` attaches it to the parent. The parent's listener bails when `document.activeElement` is an `INPUT`, `TEXTAREA` or `SELECT`: the title, slug and every ⚙ box have working native undo and taking it away to add this would be a trade nobody asked for. `keydown` runs before Quill's `beforeinput` interceptor, so `preventDefault()` here wins.

**The buttons stop lying.** They keep `free` — which is what keeps them out of `cmds` and so out of syncBar's "is the caret in a rich field" rule — but they now have a rule of their own, `canUndo()`/`canRedo()`, repainted from the manager's `stack-item-added`/`stack-item-popped`. `.tb:disabled` also gained an `opacity`, because until now nothing in that bar was ever disabled on its own and the rule set only a cursor.

**The canvas is a server render, not a second renderer.** `POST /admin/canvas` (`admin_ui.py`, `@ui_required()`, CSRF as form data) takes the blocks the browser currently holds — unsaved ones included — plus the post's `type`, and returns `render_blocks(blocks, edit=True)` inside `templates/admin/canvas.html`. **`type` is not decoration:** the canvas draws a title stub exactly when the page would (`blocks.owns_head()`, §6), and that answer depends on the content type, which the route has no other way of knowing; `admin.js` reads it from `#editor-data`, where it already sat. It carries no `meta` and no `featured_media`, so the title banner above is a **Preview**-only thing — `_preview_post()` has both and renders the real `post.html`, a standalone document linking `site.css` and `static/canvas.css`. `admin.js` puts that in `iframe#canvas` via `srcdoc`. Preview and published page therefore cannot drift. The iframe is not decoration: every `admin.css` rule is scoped to `body.admin`, which would be an ancestor of an inline canvas and would silently repaint `.specs`, `.card` and the lead form.
...
- **`splitAt()` adds no trailing paragraph inside a column.** On the page, `/` inserts the chosen section *and* an empty `rich_text` under it, because a page is a document you keep writing down. A column is a slot you place things in, so there the paragraph goes in only when the split left real text behind. **The caret lands on the section either way** (client, 2026-09-22): you asked for a Hero, so the next thing you type is its heading, and the trailing paragraph is still there to carry on in. It used to land in that trailing paragraph on the page and on the section in a column, which is one rule too many for the same keystroke.
...
the chosen section goes next, and what is after it becomes a second `rich_text`. If the `/` line
...
tests/test_offline.py    always runs — slugify, validate_blocks, render_blocks, JWT matrix, the /media route,
                         ...
                         never live, that every state-changing route is accounted for, and the
                         detail page's head -- `_render_post()` renders post.html through base.html
                         with db.settings/get_menu/public._service_nav stubbed, which is the only
                         way the page head is exercised at all
...
- **A honeypot is the whole bot defence on the contact form.** `contact_form.html` renders a hidden `website` field and `api_create_lead()` thanks-and-drops any POST that fills it; there is no rate limit, no captcha and no duplicate detection. That is deliberate while the site is in development and no spam has arrived. When it does, the first answer is a rate-limiting rule on `POST /api/v1/leads` **at Cloudflare**, not code here: the tunnel already fronts the public site and the edge sees the real client IP, whereas `throttle.client_ip()` sees only what `TRUSTED_PROXIES` tells it to believe — get that wrong and the limit becomes a global cap on the contact form for every visitor at once, the same failure `ADMIN_NETWORKS` carries (§12). If an in-app limit is ever wanted anyway, `throttle.retry_after()/record_failure()` already take a key, a limit and a window; do **not** audit a throttled lead — the client wants Activity clean of bot noise, which is the same requirement that keeps the stress harness's own attackers out of it (§12). The field caps above are the part that is not optional, and they are storage safety rather than spam defence.
...
- Undo groups by time, not by intent, for **typing**: the manager's `captureTimeout` is 500 ms, so a continuous run of keystrokes is one step, as Quill's own `delay:1000` always made it. Discrete actions do not group — `markStep()` puts a boundary on them.
- **An action in the ~120 ms before the deferred seed is not undoable.** With a realtime room `seedDoc()` waits 4 s to learn whether a peer already has a document; an edit whose `reconcileOut` tick lands *after* that seed is written into the seed itself (under `YSEED`, untracked) and the tick that follows finds nothing to diff. Earlier edits are safe — their tick reaches `YB` first and `seedDoc` then early-returns. Measured in a browser: a section moved at exactly 4.0 s left an empty undo stack.
- **Two actions inside one 120 ms `reconcileOut` window are one undo step**, because that debounce is what decides transaction boundaries and `stopCapturing()` cannot split a transaction that has already been merged. Two deliberate clicks are never that fast; a script driving the editor can be.
