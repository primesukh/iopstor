...
The knowledge graph (`graphify-out/`, gitignored) has two halves. Its code nodes are re-extracted by git hooks (`graphify hook install`: `post-commit`, `post-checkout`) after every commit and branch switch, with no LLM, so they follow the checked-out branch on their own. Its prose nodes and community labels came from an LLM pass that **is no longer run** (2026-09-10): re-reading every document to rebuild a description of it cost more than writing the description, and the description was the thing going stale. The documentation is kept current by hand instead — in the PR that changes the code, and in `/after-merge`, which audits `.claude/` against the merged diff so the map never drifts more than one PR behind. Anything that drifted goes into its own `chore/claude-sync` PR. Query the graph for the shape of the code; read the file for anything about prose.
