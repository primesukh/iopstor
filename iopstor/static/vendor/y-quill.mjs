/* y-quill 1.0.0, src/y-quill.js verbatim. One line changed: its bare `import * as Y from 'yjs'`
   became the relative path below, so there is no import map and no way to end up with two copies of
   Yjs -- ES modules are identified by resolved URL, and there is now exactly one. The awareness
   import in its jsdoc is a comment; the constructor's third argument is optional and guarded, which
   is where in-paragraph carets would go later. */
/**
