...
**The code half of the graph maintains itself; the prose half is refreshed only on `main`.** Git hooks installed by `graphify hook install` (`.git/hooks/post-commit`, `post-checkout`) re-extract changed code files in the background after every commit and branch switch (not after a `git pull`) — no LLM, a few seconds, log in `~/.cache/graphify-rebuild.log` — so the code nodes follow whatever branch is checked out. `.sql` files contribute nothing until `tree_sitter_sql` is installed beside graphify. The doc nodes (`CLAUDE.md`, `docs/*.md`, `.claude/docs/*.md`) and the community labels come from the LLM pass. **The two halves update independently, and only one of them costs anything.** After a `git pull` or a merge, `graphify update .` re-extracts the changed code with the AST — no LLM, no subagents, seconds — and is all a code-only merge needs; `/graphify --update` is the separate, expensive half that refreshes the doc nodes and the labels, so say what it will cost and let the user decide rather than firing it off. **Neither command ever takes a subdirectory**: a path like `./docs` re-roots the manifest there, every tracked file outside it reads as deleted, and its nodes are pruned — the path is always `.`. Never run `/graphify` or `graphify label` on a branch: a branch may be abandoned or reworked, and labelling it burns tokens on a shape that may never land. Query a slightly stale graph, then confirm against source.
