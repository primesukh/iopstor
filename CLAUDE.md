...
iopstor/templates/       base.html post.html archive.html 404.html checkout.html _card.html (the one card macro), blocks/<type>.html (21, each a full-width <section>), admin/*.html
...
iopstor/static/site.js   the public site's ONLY first-party script: the sliding row (.pl-rail: testimonials, and product lists since 2026-09-23) -- drift, arrows, dots, click-drag that leaves a link card clickable. The row is a native overflow-x scroller without it, so the controls ship `hidden` and this unhides them.
                         Second job (2026-09-24): an IntersectionObserver that adds .fx-wait to a counting figure (.fx-view, a Numbers band's count_in_view) while it is off screen
...
migrations/              0000_bootstrap.sql (run once by hand in Studio) + NNNN_name.sql applied by `flask migrate`; repair_schema_migrations.sql, purge_test_audit_rows.sql and repair_realtime_tenant.sql are hand-run, not steps
...
- Theme changes go in `static/site.css` (shared by the public site, `body.admin` and the editor canvas; admin extras in `static/admin.css`, canvas chrome in `canvas.css`); no CSS framework, no build step, and **no JS on the public site except `static/site.js`**, which drives the sliding row (testimonials 2026-09-16; product lists joined the same row 2026-09-23 — `blocks._rail()` decides which lists) and, since 2026-09-24, when a counting figure starts (a Numbers band's *Start counting when it scrolls into view*; the script adds `.fx-wait`, the template never does, so no script = counts at load), and nothing else. A third public feature does not get to join it without a `design.md` §9 line saying so. The client's mock in `website_assets/mock-website.html` is the design reference, and the client's later decisions in `requirements.md` override it (white header, price on the Buy button). Verify with `/theme-check`. Public forms post plain HTML to `/api/v1/leads` and get redirected back with `?sent=1`.
...
- URL scheme: pages at `/<slug>` (slug `home` = `/`), others at `/<url_prefix>/<slug>`, hierarchical types at `/<prefix>/<parent>/<slug>`,
  term archives at `/<taxonomy>/<term>`, `/<prefix>/<slug>/checkout` for a product with a price, and every resolvable URL + `.md` as its Markdown twin. `checkout` and `index` are reserved slugs (by routing order). **`admin`, `api`, `media`, `static`, `healthz`, `feed` and `sitemap` are reserved *first segments*** (`db.RESERVED_SEGMENTS`): a blueprint owns them, so a post claiming one cannot load at all — `db.reserved()` refuses it on save (a post slug only when its type has no `url_prefix`; a `url_prefix` or taxonomy slug always) and `_indexable()` keeps any older one out of every crawler file. Slugs are unique per post type.
...
- SEO output (meta, canonical, Open Graph, JSON-LD, sitemap, robots, llms.txt, llms-full.txt, `.md` twins, RSS) is server-rendered from `seo.py` + `public.py`; keep it there. **The slug is the page and the extension is the file** (2026-09-21): `/sitemap` and `/feed` are rendered pages for a person (the footer links them), `/sitemap.xml` and `/feed.xml` are the machine files (robots.txt and `<head>` autodiscovery point at them). `/robots.txt`, `/llms.txt` and `/llms-full.txt` **keep their filenames and get no page** — RFC 9309 fixes robots.txt at that path and llms.txt is discovered by its name, so renaming either breaks discovery silently. No XSLT anywhere: Chrome removes it 2026-11-17. `_indexable()` is the one gate for **everything a crawler reads** — sitemap, llms, the feed, every `.md` twin and the lists inside them — and it asks three things: has a path, that path is not one the app owns, not `noindex`. `robots.txt` deliberately names no admin path.
...
- Ponytail mode is on for this repo: fewest files, stdlib first, mark deliberate ceilings with `# ponytail:` comments (43 in `iopstor/*.py`, plus 6 in `site.css` and 1 in `site.js`; `/ponytail-debt` lists them). Non-trivial logic gets one small pytest test in `tests/test_offline.py` when it can run without Supabase.
